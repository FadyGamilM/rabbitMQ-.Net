...
channel.QueueBind(
   ...
   exchange: "TopicRoutingExchange",
   // the routing key 
   routingKey: "user.#" // any thing starts with user 
);
...
consumer.Received += (model, encodedMsg) => {
   ...
   System.Console.WriteLine($"[RECIEVED MESSAGE By User Microservice] => {message}");
};
...
System.Console.WriteLine("User microservice starts consuming");
